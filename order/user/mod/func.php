...
function _getMenuList($shop_id){
	$sql = "SELECT * FROM `menu` 
			...
			AND `stop_reservation_id` IS NULL
			ORDER BY `menu_order`";
	$res = _mysqlquery($sql);
	$list = array();
...
function _HtmlSelectTakeoutTimesToday($time = 0){
	if(!$time){
		$html2 .= '<option value="0">即時</option>';
	}
	for ($m=10; $m <=60 ; $m = $m+10) { 
		if($m<$time)continue;
		$html2 .= '<option value="'.$m.'" >'.$m.'分後</option>';
	}
	$html = "<select class='form-control' name='today_takeout_times'>".$html2."</select>";
	return $html;
}

function _getTakeOutTime($menu_id,$shop_id){
	$week_no = date("w");
...
//営業終了時間
function _getBusinessHoursFrom($shop_id){
	$week_no = date("w");
	$sql = "SELECT * FROM `store_week_time` WHERE `shop_id`='$shop_id' AND `week_no`=$week_no ";
	$res = _mysqlquery($sql);
	$col = _mysqlfetcharray($res);
	return date("H:i",strtotime($col["business_hours_from"]));
}

//営業終了時間
function _getBusinessHoursTo($shop_id){
	$week_no = date("w");
	$sql = "SELECT * FROM `store_week_time` WHERE `shop_id`='$shop_id' AND `week_no`=$week_no ";
	$res = _mysqlquery($sql);
	$col = _mysqlfetcharray($res);
	return date("H:i",strtotime($col["business_hours_to"]));
}
